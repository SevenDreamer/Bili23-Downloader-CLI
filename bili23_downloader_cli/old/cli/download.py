...
class DownloadUtils:
    ...
    def get_video_durl_via_api(self):
        try:
            if self.type == "video":
                url = api.download_api(APIType.Video, bvid=self.info["bvid"], cid=self.info["cid"])
    ...
    def get_video_durl(self, quality_id):
        ...
        self.audio_durl = [
            i for i in temp_audio_durl if (i["id"] - 30200) == Info.quality or (i["id"] - 30200) < Info.quality
        ][0]["baseUrl"]
    ...
    def onDownload(self, progress, speed, size):
        print(
            "\r{}% | {}{}  |  {}   {}".format(progress, "█" * (progress // 5), " " * (20 - progress // 5), speed, size),
            end="",
            flush=True,
        )
